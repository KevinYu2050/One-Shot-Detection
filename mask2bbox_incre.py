...
def get_contours(img):
    ...
    _, gray=cv2.threshold(gray,150,255,cv2.THRESH_BINARY) #大于150的像素变为255，其余变为0
...
def main():
    ...
    root_dir = './output/incre/test_conv.2/{}'.format(obj_name)
    ...
    save_dir = './vis/incre.conv.2/{}'.format(obj_name)
    ...
    for filename in filenames:
        ...
        if not bboxes:
            ...
            # Determines if the bbox is qualified according to scale/activation info
            for bbox in bboxes:
                if bbox:
                    # @param: flag size must be greater than 1/2500 of the image size
                    if bbox[1]-bbox[0]> width/70 and bbox[3]-bbox[2]>height/70:
                        qualified_boxes.append(bbox)
            
            print(len(qualified_boxes))
    ...
    print('bbox_num:', bbox_num)
    with open('./det_files/incre_video_test_conv.2_obj{}.json'.format(obj_name), 'w') as f:
